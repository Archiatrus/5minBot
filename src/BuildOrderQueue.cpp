...
std::string BuildOrderQueue::getQueueInformation() const
{
    ...
    for (size_t i(0); i<reps; i++)
    {
		...
		else if (type.getUnitTypeID()!=0)
		{
			ss << m_bot.Observation()->GetUnitTypeData()[type.getUnitTypeID()].name << "\n";
		}
    }
...
int BuildOrderQueue::howOftenQueued(sc2::UNIT_TYPEID type) const
{
	int numOfType = 0;
	// for each unit in the queue
	for (size_t i(0); i<m_queue.size(); i++)
	{
		const BuildType & bType = m_queue[i].type;
		if (type == bType.getUnitTypeID())
		{
			numOfType++;
		}
	}
