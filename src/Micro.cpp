...
void Micro::SmartAttackUnit(const sc2::Units & attacker, const sc2::Unit * target, CCBot & bot, bool queue)
{
	...
	{
		if (!(a->orders.empty()) && (a->orders.back().target_unit_tag == target->tag || (a->orders.back().ability_id == sc2::ABILITY_ID::ATTACK && Util::Dist(a->orders.back().target_pos, target->pos) < 1.0f)))
		{
			continue;
		}
		//Distance to target
...
{
    BOT_ASSERT(attacker != nullptr, "Attacker is null");
	if (attacker->orders.empty() || attacker->orders.back().ability_id != sc2::ABILITY_ID::ATTACK || Util::Dist(attacker->orders.back().target_pos,targetPosition) > 1.0f)
	{
